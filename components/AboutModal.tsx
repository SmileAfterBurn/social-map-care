...
import { X, Heart, Share2, Github, Mail, Globe, Award, Wallet, ArrowRight, Target, Users, ShieldCheck, Briefcase, Copy, ExternalLink, Laptop, PlayCircle, Scale, Terminal, Shield, FileText, Handshake } from 'lucide-react';
...
interface AboutModalProps {
  ...
  onOpenPrivacy?: () => void;
  onOpenTerms?: () => void;
  initialTab?: 'about' | 'donate' | 'partners' | 'legal';
}

export const AboutModal: React.FC<AboutModalProps> = ({ onClose, onOpenPresentation, onOpenPrivacy, onOpenTerms, initialTab = 'about' }) => {
  const [activeTab, setActiveTab] = useState<'about' | 'donate' | 'partners' | 'legal'>(initialTab);
        ...
        <div className="p-4 md:p-8 overflow-y-auto custom-scrollbar flex-1">
           {activeTab === 'about' && (
             <div className="space-y-6 animate-in fade-in slide-in-from-bottom-2 duration-300">
               ...
               <div className="bg-gradient-to-br from-indigo-600 to-blue-700 rounded-3xl p-5 text-white shadow-xl relative overflow-hidden group">
                 <div className="absolute top-0 right-0 p-4 opacity-10 group-hover:scale-110 transition-transform duration-500">
                   <Handshake size={80} />
                 </div>
                 <div className="relative z-10">
                   <h4 className="font-black uppercase tracking-widest text-[10px] mb-1 text-indigo-200">Можливості співпраці</h4>
                   <h3 className="text-lg font-black mb-3 leading-tight uppercase">Допомагайте разом з нами</h3>
                   <p className="text-xs text-white/80 mb-4 max-w-[240px] font-medium leading-relaxed">Шукаємо партнерів для актуалізації даних та масштабування ШІ-сервісів.</p>
                   <button 
                     onClick={() => setActiveTab('partners')}
                     className="flex items-center gap-2 px-5 py-2 bg-white text-indigo-700 rounded-xl text-[10px] font-black uppercase tracking-widest hover:bg-indigo-50 transition-all shadow-lg active:scale-95"
                   >
                     Стати партнером <ArrowRight size={14} />
                   </button>
                 </div>
               </div>
             ...
             <div className="space-y-4 md:space-y-6 animate-in fade-in slide-in-from-bottom-2 duration-300">
               <div className="bg-slate-900 text-white p-5 md:p-6 rounded-[2rem] shadow-xl relative overflow-hidden border border-white/10">
                 ...
                 <div className="flex flex-col gap-2 relative z-10">
                   ...
                   <a href="mailto:chernov.illia@icloud.com" className="w-full py-3 bg-teal-600 text-white rounded-xl font-black uppercase text-[10px] tracking-widest hover:bg-teal-700 transition text-center active:scale-95 flex items-center justify-center gap-2"><Mail size={16} /> Зв'язатися</a>
                 </div>
               </div>
               ...
               <div className="p-6 bg-slate-50 rounded-3xl border border-slate-100 border-dashed text-center">
                  ...
                  <p className="text-xs text-slate-500 font-bold uppercase tracking-widest">Також відкриті до співпраці з державними органами та волонтерськими хабами</p>
               </div>
             </div>
           )}
